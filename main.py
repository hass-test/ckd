...
st.image('logo.png', caption='Chronic Kidney Disease Dialysis Prediction', use_column_width=True)
...
with st.sidebar:
    ...
    potassium = st.slider(label="", min_value=2.00, max_value=8.00, step=0.01)
    ...
    GFR_delta_last_4_months = st.slider(label="GFR(GFR delta last 4 months)", min_value=-16.00,
                                        max_value=36.00, step=0.01)
    GFR_delta_last_year = st.slider(label="GFR(GFR delta last year)", min_value=-19.00,
                                    max_value=50.00, step=0.01)
    GFR_standard_deviation_last_4_month = st.slider(label="GFR(GFR standard deviation last 4 month)",
                                                    min_value=0.00,
                                                    max_value=16.00, step=0.01)
    GFR_standard_deviation_last_year = st.slider(label="GFR(GFR standard deviation last year)", min_value=0.00,
                                                 max_value=16.00, step=0.01)
    last_GFR = st.slider(label="GFR(last GFR)", min_value=15.00,max_value=35.00)
...
#     specific_gravity_standard_deviation = st.text_input(label='(SGSTDV)',value='0.0000045')
